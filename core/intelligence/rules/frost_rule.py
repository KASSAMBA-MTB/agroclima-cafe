...
class FrostRule(BaseRule):
    ...
    description = "Avaliação multicritério do risco de geadas."
    ...
    def evaluate(self, context):
        ...
        value, factor = self._temperature_score(
            temperature
        )
        ...
        value, factor = self._humidity_score(
            humidity
        )
        ...
        value, factor = self._cloud_score(
            cloud
        )
        ...
        value, factor = self._altitude_score(
            altitude
        )
        ...
        value, factor = self._season_score(
            analysis_date.month
        )
        ...
        value, factor = self._history_score(
            historical
        )
    ...
    def _temperature_score(self, temperature):
        """
        ...
        if temperature <= 0:
            return 40, "Temperatura ≤ 0°C"

        if temperature <= 2:
            return 30, "Temperatura entre 0°C e 2°C"

        if temperature <= 5:
            return 20, "Temperatura entre 2°C e 5°C"

        if temperature <= 8:
            return 10, "Temperatura entre 5°C e 8°C"
    ...
    def _humidity_score(self, humidity):
        ...
        if humidity >= 90:
            return 15, "Umidade elevada"

        if humidity >= 80:
            return 10, "Umidade alta"

        if humidity >= 70:
            return 5, "Umidade moderada"

        return 0, None

    def _wind_score(self, wind):
        """
        ...
        if wind <= 5:
            return 10, "Ventos fracos"

        if wind <= 10:
            return 5, "Ventos moderados"

        return 0, None

    def _cloud_score(self, cloud):
        """
        ...
        if cloud <= 50:
            return 5, "Pouca nebulosidade"

        return 0, None

    def _altitude_score(self, altitude):
        """
        ...
        if altitude >= 1200:
            return 10, "Altitude muito elevada"

        if altitude >= 900:
            return 7, "Altitude elevada"

        if altitude >= 700:
            return 4, "Altitude intermediária"

        return 0, None

    def _season_score(self, month):
        """
        ...
        if month in (5, 9):
            return 5, "Transição sazonal"

        return 0, None

    def _history_score(self, historical):
        """
